...
const FloatingChatWidget = ({ 
  ...
  initialTheme = "light",
  position = "right",
  primaryColor = "#4F46E5", // More vibrant indigo
  ...
  const [isChatOpen, setIsChatOpen] = useState(isInitiallyOpen);
  const [isTyping, setIsTyping] = useState(false);
  const [suggestions, setSuggestions] = useState([]);
  const [history, setHistory] = useState({ messages: [], feedback: {} });
  const [preferences, setPreferences] = useState({ theme: initialTheme });
  ...
  useEffect(() => {
    ...
    if (savedHistory.messages && savedHistory.messages.length > 0) {
      ...
      setHistory({ messages: [welcomeMsg], feedback: {} });
    }
  ...
  useEffect(() => {
    if (chatWindowRef.current && history.messages.length > 0) {
      const botMessages = chatWindowRef.current.querySelectorAll(".chat-message-bot");
  ...
  // Apply custom color styles with enhanced gradients
  const buttonGradient = {
    light: `background-image: linear-gradient(135deg, #7C3AED, #DB2777)`, // More vibrant violet to pink
    dark: `background-image: linear-gradient(135deg, #7C3AED, #DB2777)`,  // Keeping same for dark mode
  };
  ...
  // New pulsating animation for chat button
  ...
  // Update toggleChat function to call the callback
  ...
  return (
    <>
      ...
      <div 
        className={`fixed bottom-6 ${isRightPositioned ? 'right-6' : 'left-6'} z-50`}
      ...
      >
        <AnimatePresence>
          {isChatOpen && (
            <motion.div
              ...
              } rounded-2xl overflow-hidden flex flex-col mb-4 border ${
        ...
        {/* Chat Button with improved styling and visibility */}
        <motion.div
          className="relative"
        >
