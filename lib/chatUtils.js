...
const getGeminiResponse = async (query, localData) => {
  try {
    ...
    const relevantData = localData.filter(item => {
      const itemText = (item.question + ' ' + item.answer).toLowerCase();
      return queryWords.some(word => itemText.includes(word));
    });

    // If no relevant data found, send a minimal context
    const dataToSend = relevantData.length > 0 ? relevantData : 
      [{ 
        question: "About Abhinav Academy",
        answer: "Abhinav Academy is an educational institution in Gadhinglaj focusing on competitive exam preparation."
      }];
      console.log("Relevant data for Gemini API:", dataToSend);
    ...
    const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_API_KEY}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `Give a short, friendly response like a teacher/guide in plain text format. Use this relevant information to answer accurately:

            CONTEXT: ${JSON.stringify(dataToSend)}

            QUESTION: ${query}
            Don't Give any wrong information because it will be used for educational purposes.
            Keep responses focused on Abhinav Academy's educational services. For non-educational queries, provide a polite deflection.`
          }],
        }],
      }),
    });
