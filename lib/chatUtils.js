  ...
  try {
    ...
    const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_API_KEY}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `Answer like a friendly teacher in plain text and very short/small as you can.
            CONTEXT: ${JSON.stringify(dataToSend)}
            QUESTION: ${query}
            IMPORTANT: Use the CONTEXT to answer and your knowledge if needed. If you cant understand QUESTION then as user to clarify the question,dont give him answer. Give clear, accurate, and genuine info. Focus on Abhinav Academy and study-related topics. Math/study questions are allowed. Deflect unrelated queries politely. Keep it very short (min 1, max(2-3) sentences).`
          }]
